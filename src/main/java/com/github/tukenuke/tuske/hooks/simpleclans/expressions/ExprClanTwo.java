...
import com.github.tukenuke.tuske.util.SkriptCompat;
...
public class ExprClanTwo extends SimpleExpression<Clan>{
	...
	public boolean init(Expression<?>[] arg0, int arg1, Kleenean arg2, ParseResult arg3) {
		if (!(SkriptCompat.isCurrentEvent(AllyClanAddEvent.class) || SkriptCompat.isCurrentEvent(AllyClanRemoveEvent.class) || SkriptCompat.isCurrentEvent(RivalClanAddEvent.class) || SkriptCompat.isCurrentEvent(RivalClanRemoveEvent.class))){
			Skript.error("'event-clan-two' can only be used in 'On (Ally|Rival) clan (add|remove)' event.", ErrorQuality.SEMANTIC_ERROR);
			return false;
		}
		if (SkriptCompat.isCurrentEvent(AllyClanAddEvent.class))
			this.e = 1;
		else if (SkriptCompat.isCurrentEvent(AllyClanRemoveEvent.class))
			this.e = 2;
		else if (SkriptCompat.isCurrentEvent(RivalClanAddEvent.class))
			this.e = 3;
		else if (SkriptCompat.isCurrentEvent(RivalClanRemoveEvent.class))
			this.e = 4;
		return true;
	}
