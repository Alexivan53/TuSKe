...
public class ExprLandflag extends SimpleExpression<Boolean>{
	...
	public void change(Event e, Object[] delta, Changer.ChangeMode mode){
		...
		if (ol != null && lf != null){
			...
			try {
				// Prefer API saveOwnedLand if available (avoids Ebean at compile time)
				Landlord.getInstance().getClass().getMethod("saveOwnedLand", ol.getClass()).invoke(Landlord.getInstance(), ol);
			} catch (Throwable ignored) {
				try {
					// Fallback: older Landlord versions
					Landlord.getInstance().getClass().getMethod("saveLand", ol.getClass()).invoke(Landlord.getInstance(), ol);
