...
public class SyntaxInfo {
	...
	public String id, name, return_type, dependency;
	public String[] since, desc, examples, patterns, usage, changers, event_values;
	public Boolean cancellable; //It needs to be null for non event syntax

	public SyntaxInfo(SyntaxElementInfo info) {
		Class<?> c = com.github.tukenuke.tuske.util.ReflectionUtils.getField(info.getClass(), info, "c");
		if (c == null)
			c = info.getClass();
	...
	public SyntaxInfo(ExpressionInfo info, Class[] classes) {
		...
		Class<?> clz = ReflectionUtils.getField(info.getClass(), info, "c");
		Expression expr = clz != null ? (Expression) ReflectionUtils.newInstance((Class) clz) : null;
		try {
			for (Changer.ChangeMode mode : Changer.ChangeMode.values()) {
				if (expr != null && Changer.ChangerUtils.acceptsChange(expr, mode, classes))
	...
	public SyntaxInfo(SkriptEventInfo info, EventValuesGetter getter) {
		...
		try {
			Object s = info.getSince();
			since = (s instanceof String[]) ? (String[]) s : (s != null ? new String[]{s.toString()} : null);
		} catch (Throwable t) {
			Object s = com.github.tukenuke.tuske.util.ReflectionUtils.invokeMethod(
					com.github.tukenuke.tuske.util.ReflectionUtils.getMethod(info.getClass(), "getSince"), info);
			since = (s instanceof String[]) ? (String[]) s : (s != null ? new String[]{s.toString()} : null);
		}
		if (getter != null) {
	...
	public SyntaxInfo(JavaFunction info) {
		...
		Object jfSince = info.getSince();
		since = (jfSince instanceof String[]) ? (String[]) jfSince : (jfSince != null ? new String[]{jfSince.toString()} : null);
