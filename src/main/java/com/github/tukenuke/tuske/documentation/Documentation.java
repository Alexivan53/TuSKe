...
public class Documentation implements Runnable{
	...
	public void run() {
		...
		for (SkriptEventInfo info : Skript.getEvents()) {
			Class<?> clz = com.github.tukenuke.tuske.util.ReflectionUtils.getField(info.getClass(), info, "c");
			if (clz == null) clz = info.getClass();
			addSyntax(getAddon(clz).getEvents(), new SyntaxInfo(info, getter));
		}
		for (SyntaxElementInfo info : Skript.getConditions()) {
			Class<?> clz = com.github.tukenuke.tuske.util.ReflectionUtils.getField(info.getClass(), info, "c");
			if (clz == null) clz = info.getClass();
			if (EffectSection.class.isAssignableFrom(clz)) //Separate effect sections to effects instead of conditions
				addSyntax(getAddon(clz).getEffects(), new SyntaxInfo(info));
			else
				addSyntax(getAddon(clz).getConditions(), new SyntaxInfo(info));
		}
		...
		Skript.getExpressions().forEachRemaining(info -> {
			Class<?> clz = com.github.tukenuke.tuske.util.ReflectionUtils.getField(info.getClass(), info, "c");
			if (clz == null) clz = info.getClass();
			addSyntax(getAddon(clz).getExpressions(), new SyntaxInfo(info, types));
		});
		log.clear();
		log.stop();
