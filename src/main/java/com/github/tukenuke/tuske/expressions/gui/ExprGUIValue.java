...
public class ExprGUIValue extends SimpleExpression<Object>{
			...
			switch (type) {
				...
				case 11:
					if (((InventoryCloseEvent) e).getInventory() != null)
						return new String[]{((InventoryCloseEvent) e).getView().getTitle()};
					break;
				case 13:
					return new GUIInventory[]{gui};
			}
	...
	public Class<?>[] acceptChange(final Changer.ChangeMode mode) {
		if (SkriptCompat.isCurrentEvent(InventoryCloseEvent.class))
			return null;
		if (type == 6 || type == 7) {
			if (!isDelayed) {
				if (changer == null) {
					...
					if (slotClz != null)
						changer = Classes.getExactClassInfo(slotClz).getChanger();
				}
				return changer != null ? changer.acceptChange(mode) : null;
			}
