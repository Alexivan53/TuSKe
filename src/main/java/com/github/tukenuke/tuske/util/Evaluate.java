	...
	public void evaluate(String code, Event e, Variable results, boolean parseString, Config currentScript, List<Argument<?>> args, TriggerItem source, boolean runWithSafety) {
		if (code != null && !code.isEmpty()) {
			...
			try {
				...
				if (parseString) {
					ReflectionUtils.setField(ScriptLoader.class, null, "currentScript", currentScript);
				...
				Config c;
				try {
					c = Config.class.getConstructor(java.io.InputStream.class, String.class, boolean.class)
							.newInstance(new ByteArrayInputStream(code.getBytes(StandardCharsets.UTF_8)), "TuSKe/evaluate.sk", true);
				} catch (Throwable ctorNew) {
					try {
						c = Config.class.getConstructor(String.class, String.class, boolean.class, boolean.class, String.class)
								.newInstance(code, "TuSKe/evaluate.sk", true, false, ":");
					} catch (Throwable ignored) {
						throw new RuntimeException("No compatible Skript Config constructor found", ctorNew);
					}
				}
				...
				ReflectionUtils.setField(ScriptLoader.class, null, "currentScript", c);
				SkriptCompat.setCurrentEvent("evaluate effect", e.getClass());
				...
				SkriptCompat.deleteCurrentEvent();
				ReflectionUtils.setField(ScriptLoader.class, null, "currentScript", null);
