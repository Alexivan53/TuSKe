...
public abstract class EffectSection extends Condition {
	...
	public void loadSection(String name, boolean setNext, Class<? extends Event>... events){
		if (section != null && name != null && events != null && events.length > 0) {
			String previousName = SkriptCompat.getCurrentEventName();
			Class<? extends Event>[] previousEvents = SkriptCompat.getCurrentEvents();
			Kleenean previousDelay = (Kleenean) com.github.tukenuke.tuske.util.ReflectionUtils.getField(ScriptLoader.class, null, "hasDelayBefore");
			SkriptCompat.setCurrentEvent(name, events);
			loadSection(setNext);
			SkriptCompat.setCurrentEvent(previousName, previousEvents);
			com.github.tukenuke.tuske.util.ReflectionUtils.setField(ScriptLoader.class, null, "hasDelayBefore", previousDelay);
		}
	}
